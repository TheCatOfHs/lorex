...
def batch(cifs):
    ...
    counter, buffer = 0, []
    for idx, poscar in enumerate(cifs):
...
if __name__ == '__main__':
    ...
    train_cifs = [f'CIF2D_no_vdW/{i}.cif' for i in train_cifs]
    valid_cifs = [f'CIF2D_no_vdW/{i}.cif' for i in valid_cifs]
    test_cifs = [f'CIF2D_no_vdW/{i}.cif' for i in test_cifs]
    ...
    train_atom_fea, train_nbr_fea, train_nbr_fea_idx = batch(train_cifs)
    valid_atom_fea, valid_nbr_fea, valid_nbr_fea_idx = batch(valid_cifs)
    test_atom_fea, test_nbr_fea, test_nbr_fea_idx = batch(test_cifs)
